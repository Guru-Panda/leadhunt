...
export default function LeadsPage() {
  ...
  const [refreshDialog, setRefreshDialog] = useState<{ open: boolean; unwantedCount: number } | null>(null)
  ...
  const confirmRefresh = async () => {
    if (!filters.strategy_id || !refreshDialog) return
    setRefreshing(true)
    ...
    try {
      ...
      setRefreshNotice('Refresh failed. Check Monitor page for errors.')
    } finally {
      setRefreshing(false)
      setTimeout(() => setRefreshNotice(''), 8000)
    }
  }
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {refreshDialog?.open && (
          <RefreshConfirmDialog
            unwantedCount={refreshDialog.unwantedCount}
            strategyName={strategies.find((s) => s.id === filters.strategy_id)?.title ?? 'this strategy'}
            onCancel={() => setRefreshDialog(null)}
        ...
        <div className="flex-1 overflow-auto">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <div className="text-lg font-bold text-gradient mb-1">No leads yet</div>
              <div className="text-sm text-gray-500 max-w-xs">Create a strategy and leads will appear here automatically</div>
            </div>
          ) : (
...
function RefreshConfirmDialog({
  unwantedCount, strategyName, onCancel, onConfirm,
}: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onCancel}>
      ...
      >
        ...
        <div className="flex gap-2 justify-end">
          ...
          <button onClick={onConfirm} className="btn-primary text-sm flex items-center gap-1.5">
            Refresh leads
          </button>
        </div>
      </div>
